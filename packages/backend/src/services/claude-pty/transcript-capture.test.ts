...
describe('transcript capture', () => {
  ...
  it('skips slash-command machinery (<command-name>, <local-command-stdout>, caveat)', async () => {
    // Regression: typing `/exit` (or any slash command) logs synthetic `user`
    // lines wrapping the command + its output. Those leaked into the history/pins
    // panel as raw markup; capture must drop them and keep only real turns.
    const sid = 'sess-cap-cmd';
    insertSession(sid);
    const dir = mkdtempSync(path.join(tmpdir(), 'cap-'));
    const tfile = path.join(dir, 'claude-cmd.jsonl');
    writeTranscript(tfile, [
      { type: 'user', uuid: 'cu1', message: { role: 'user', content: '안녕' } },
      { type: 'assistant', uuid: 'ca1', message: { role: 'assistant', model: 'claude-opus-4-8', content: [{ type: 'text', text: '안녕하세요!' }] } },
      { type: 'user', uuid: 'cu2', message: { role: 'user', content: '<local-command-caveat>Caveat: messages below were generated by the user…</local-command-caveat>' } },
      { type: 'user', uuid: 'cu3', message: { role: 'user', content: '<command-message>exit</command-message>\n<command-name>/exit</command-name>\n<command-args></command-args>' } },
      { type: 'user', uuid: 'cu4', message: { role: 'user', content: '<local-command-stdout>Catch you later!</local-command-stdout>' } },
    ]);

    await startCapture(sid, null);
    await fireStop(sid, tfile, 'claude-cmd');

    const r = rows(sid);
    expect(r.map((x) => x.content)).toEqual(['안녕', '안녕하세요!']);
    expect(r.some((x) => x.content.includes('command'))).toBe(false);
